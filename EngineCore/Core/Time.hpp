...
namespace ME::Core::Clock
{
	...
	{
		inline constexpr int64 Max = 0xFFFFFFFFFFFFFFFFULL / 2;
		inline constexpr int64 Min = 0x0000000000000000LL;
	...
	public:
		float64 AsNanoseconds() const
		{
			...
			return (static_cast<float64>(m_Ticks) / static_cast<float64>(m_Source->GetFrequency())) * 1e9L;
		}
		float64 AsMicroseconds() const
		{
			ME_ASSERT(m_Source, "TimeSource not set!");
			return (static_cast<float64>(m_Ticks) / static_cast<float64>(m_Source->GetFrequency())) * 1e6L;
		}
		float64 AsMilliseconds() const
		{
			ME_ASSERT(m_Source, "TimeSource not set!");
			return (static_cast<float64>(m_Ticks) / static_cast<float64>(m_Source->GetFrequency())) * 1e3L;
		}
		float64 AsSeconds() const
		{
			ME_ASSERT(m_Source, "TimeSource not set!");
			return static_cast<float64>(m_Ticks) / static_cast<float64>(m_Source->GetFrequency());
		}
