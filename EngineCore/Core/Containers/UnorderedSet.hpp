...
namespace ME::Core
{
	...
	public:
		USetConstIterator(PtrType ptr, NodeType** buckets, SIZE_T num_buckets, SIZE_T bucket_index)
			: m_Ptr(ptr), m_Buckets(buckets), m_NumBuckets(num_buckets), m_BucketIndex(bucket_index) {
		}

		USetConstIterator& operator++()
		{
			if (m_Ptr && m_Ptr->NextNode)
				m_Ptr = m_Ptr->NextNode;
			else
		...
		USetConstIterator operator++(int)
		{
		...
		inline bool operator!=(const USetConstIterator& it) const
		{
			return m_Ptr != it.m_Ptr;
		}
	...
	public:
		...
		using ConstIterator = USetConstIterator<UnorderedSet>;
	...
	private:
		...
		ConstIterator PFind(const ValueType& value) const
		{
