...
namespace ME::Render
{
	ME::Core::Memory::Reference<Texture1D> Texture1D::CreateVulkan(const Texture1DSpecification& specification)
	{
		auto object = ME::Core::Memory::MakeReference<VulkanTexture1D>(specification);
		return object;
	}

	VulkanTexture1D::VulkanTexture1D(const Texture1DSpecification& specification)
		: Texture1D(specification)
	{
		Init();
	}
	...
	void VulkanTexture1D::SetData(void* data, SIZE_T size)
	{
		UpdateImage(data, size);
	}

	void VulkanTexture1D::Shutdown()
	{
	...
	void VulkanTexture1D::Bind(ME::Core::Memory::Reference<CommandBuffer> commandBuffer, ME::Core::Memory::Reference<Pipeline> pipeline)
	{
	...
	VkResult VulkanTexture1D::CreateImage()
	{
		...
		createInfo.extent.width = m_Specification.Resolution;
	...
	VkResult VulkanTexture1D::CreateImageView()
	{
		...
		createInfo.viewType = m_Specification.CubeMapCount > 0 ? VK_IMAGE_VIEW_TYPE_CUBE : VK_IMAGE_VIEW_TYPE_1D;
	...
	VkResult VulkanTexture1D::CreateSampler()
	{
	...
	VkResult VulkanTexture1D::UpdateImage(void* data, SIZE_T size)
	{
